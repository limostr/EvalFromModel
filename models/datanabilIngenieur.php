...
$Ingenieur =array(
    'Name'=>"Metrise"
    ,"parameters"=>array(
        "idcvp"=>"{Licence:database:loader:sql:#s2:prepareInit:#idcvparcoursetud}"
    )
    ,"database"=>array(
        "init"=>array(
            "idUser"=>7
                    ...
                    "s1"=>
                        array(
                                "sqlstring"=>"SELECT * FROM candidatcv WHERE idcandidat=:idUser"
                                 ,"prepare"=>array(
                                        "idUser"=>"{Licence:database:init:#idUser}"
                                    )
                                 ,"bind"=>
                                     array(
                                         "SET"=>array(
                                             "idcandidatcv"=>"{Licence:database:loader:sql:#s2:prepare:#idcandidatcv}"
                                         ),
                                         "GET"=>array(
                                             "{Licence:AutreInformations:@bacmoyenne}"=>"moyenne"
                                             ,"{Licence:AutreInformations:@idcandidatcv}"=>"idcandidatcv"
                        ...
                        array(
                                ...
                                'SelectorType'=>array(
                                    ...
                                    "template"=>array(
                                        ...
                                        ,"template"=>"<br><b><a href='#' onclick=\"ReloadForData('idcvp','{idcvp}')\">{Label}</a></b>"
                                    )
                                ),
                                'recordset'=>[]
                             )
    ...
    ,'Formule'=>array(
        "F1"=>array(
            ...
            ,'toEval'=>"
						   ...
						   ( MethodEval::SUM({Licence:SubComp:Model:BS:@Score}) * 0.5 ) -({Licence:AutreInformations:@nbrredouble})+
							2*(({Licence:SubComp:#N1:@Score} + {Licence:SubComp:#N2:@Score} * 2 + {Licence:SubComp:#N3:@Score} *2)/5) -
							0.5 * MethodEval::SUM({Licence:SubComp:Model:BM:@Score}) +
							{Licence:AutreInformations:@BBAC}+
							{Licence:AutreInformations:@BC}+
							{Licence:AutreInformations:@BD} +
							MethodEval::ExPro({Licence:database:init:#idUser})
    ...
    ,'decision'=>""
    ,'SubComp'=>[]
    ,'AutreInformations'=>array("typediplome"=>"","idcvparcoursetud"=>"","CPAYS"=>"","idlangues"=>"","idtypesdiplome"=>"","idbacplus"=>"","idcandidatcv"=>"","titre"=>"","nbrredouble"=>"0","id"=>"","BC"=>"0","credit"=>"0","BD"=>"0","BAD"=>"0","bacmoyenne"=>"0","BBAC"=>"","Type"=>"licence")
    ,'Model'=>
			array(
			/*"BC"=>array(
						'Name'=>"BC"
    ...
    ,"template"=>array(
        "ligne"=>array(
            "Score"     =>        array("Type"=>"SET","Malloc"=>"{Licence:@Score}","Value"=>"")
            ,"Credit"   =>        array("Type"=>"SET","Malloc"=>"{Licence:AutreInformations:@credit}","Value"=>"")
            ,"BAC"  =>      array("Type"=>"SET","Malloc"=>"{Licence:AutreInformations:@bacmoyenne}","Value"=>"")
            ,"N1"   =>      array("Type"=>"SET","Malloc"=>"{Licence:SubComp:#N1:@Score}","Value"=>"")
            ,"N2"   =>      array("Type"=>"SET","Malloc"=>"{Licence:SubComp:#N2:@Score}","Value"=>"")
            ,"N3"   =>      array("Type"=>"SET","Malloc"=>"{Licence:SubComp:#N3:@Score}","Value"=>"")
            ,"BS"   =>      array("Type"=>"EVAL","Malloc"=>"MethodEval::SUM({Licence:SubComp:Model:BS:@Score})","Value"=>"")
            ,"BM"   =>      array("Type"=>"EVAL","Malloc"=>"MethodEval::SUM({Licence:SubComp:Model:BM:@Score})","Value"=>"")
            ,"Moy"  =>      array("Type"=>"EVAL","Malloc"=>"MethodEval::MOYART({Licence:SubComp:Model:Moy:@Score})","Value"=>"")
        )
        ,"liste"=>array(
